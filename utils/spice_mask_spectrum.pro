...
;     Ver.2, 09-Jul-2025, Peter Young
;       Missing data are now excluded from the mask spectrum.
;-

; $Id: 2025-07-09 16:58 EDT $
...
k=where(swtch EQ 1,qual_max)

IF n_tags(windata) NE 0 THEN BEGIN
  nwin=1
  date_obs=windata.hdr.date_obs
  slit_wid=windata.hdr.slit_wid
  l1name=windata.filename
ENDIF ELSE BEGIN 
  d=spice_data(l1name)
...
FOR i=0,nwin-1 DO BEGIN
 ...
 ;
  ...
  FOR j=0,nl-1 DO BEGIN
   ...
   ;
    k=where(swtch EQ 1 AND int_img NE wd.missing,nk)
