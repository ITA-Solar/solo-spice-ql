...
;
; PURPOSE:
;	The purpose of zoom is to display a zoomed image of some 
...
;
; CALLING SEQUENCE:
;       zoom, image, xscale, yscale, xtitle = xtitle, ytitle = ytitle, $
...
;                                     - fixed bug, x/yscale was not defined in zoom_ps
;        16-May-2023: Martin Wiesmann - Uses widget_position now
;-
; save as postscript file
pro zoom_ps, event
  thisfile=dialog_pickfile(/write,file='zoom.ps')
...
; save as jpeg file
pro zoom_jpeg,event
  thisfile=dialog_pickfile(/write,file='zoom.jpg')
  if thisfile eq '' then return
  widget_control,event.top,get_uvalue=info
  wset,(*info).wid
  snapshot=tvrd()
...
pro zoom_destroy, event
 widget_control, event.top,/destroy
end

pro zoom, image, xscale, yscale, xtitle = xtitle, ytitle = ytitle, $
           xrange = xrange, yrange = yrange, group_leader=group, n_subplot=n_subplot

if n_params() eq 0 then begin
  print,'zoom: No image specified!'
endif

; xrange, yrange are passed by zoom and not passed by xdetector, xmap, etc.
...
tlb = widget_base(title='zoom', /column, mbar = menubar, $
      tlb_size_events=1)
  ...
  psmenu=widget_button(savemenu, value='Postscript', event_pro = 'zoom_ps')
  jpgmenu=widget_button(savemenu, value='JPG', event_pro = 'zoom_jpeg')
  exitmenu=widget_button(filemenu, value='Close', event_pro='zoom_destroy')
...
xmanager,'zoom',tlb,/no_block, $
  event_handler='zoom_resize',cleanup='zoom_cleanup', $
  group_leader=group
